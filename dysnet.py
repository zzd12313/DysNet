...
class Embedding(nn.Module):
    ...
    def forward(
        ...
        if torch.isnan(x).any():
            print(f"Warning: Int2c1e embedding contains NaN")
        x_scalar = self.node_lin(x)
        if edge_index.numel() > 0:
            node_dist = scatter(dist.squeeze(-1), edge_index[0], dim=0, dim_size=pos.size(0), reduce="mean")
            dist_normalized = torch.clamp(node_dist, 0.1, 10.0)
            dist_normalized = (dist_normalized - 0.1) / (10.0 - 0.1)
            pos_encoding = torch.tanh(dist_normalized * 2.0 - 1.0)
            pos_encoding = pos_encoding.unsqueeze(-1).expand(-1, x_scalar.size(-1)) * 0.0005
            x_scalar = x_scalar + pos_encoding
            if torch.isnan(pos_encoding).any():
                print(f"ERROR: pos_encoding contains NaN in Embedding - this should not happen!")
                print(f"node_dist stats: mean={node_dist.mean().item():.6f}, std={node_dist.std().item():.6f}")
                print(f"dist_normalized stats: mean={dist_normalized.mean().item():.6f}, std={dist_normalized.std().item():.6f}")
                print(f"pos_encoding stats: mean={pos_encoding.mean().item():.6f}, std={pos_encoding.std().item():.6f}")
                raise ValueError("NaN detected in position encoding - check distance normalization logic")
        if torch.isnan(x_scalar).any():
            print(f"Warning: Node linear output contains NaN")
            print(f"Input x stats: mean={x.mean().item():.3f}, std={x.std().item():.3f}")
            print(f"Linear layer weight stats: mean={self.node_lin.weight.mean().item():.3f}, std={self.node_lin.weight.std().item():.3f}")
            print(f"Linear layer bias stats: mean={self.node_lin.bias.mean().item():.3f}, std={self.node_lin.bias.std().item():.3f}")
        rbf = self.rbf(dist)
        fcut = self.cutoff_fn(dist)
        if torch.isnan(rbf).any():
            print(f"Warning: RBF output contains NaN")
            print(f"dist stats: mean={dist.mean().item():.3f}, std={dist.std().item():.3f}, min={dist.min().item():.3f}, max={dist.max().item():.3f}")
        if torch.isnan(fcut).any():
            print(f"Warning: fcut output contains NaN")
            print(f"dist stats: mean={dist.mean().item():.3f}, std={dist.std().item():.3f}, min={dist.min().item():.3f}, max={dist.max().item():.3f}")
        vec_norm = torch.linalg.vector_norm(vec, dim=-1, keepdim=True)
        vec_norm_clamped = torch.clamp(vec_norm, 1e-6, 100.0)
        vec_normalized = torch.where(
            vec_norm < 1e-6,
            torch.zeros_like(vec),
            vec / vec_norm_clamped
        )
        vec_normalized = torch.clamp(vec_normalized, -1.0, 1.0)
        try:
            rsh = self.sph_harm(vec_normalized[:, [1, 2, 0]])
        except Exception as e:
            print(f"ERROR in spherical harmonics calculation: {e}")
            print(f"vec_normalized shape: {vec_normalized.shape}")
            print(f"vec_normalized stats: mean={vec_normalized.mean().item():.6f}, std={vec_normalized.std().item():.6f}")
            print(f"vec_norm stats: mean={vec_norm.mean().item():.6f}, std={vec_norm.std().item():.6f}")
            print(f"vec_norm_clamped stats: mean={vec_norm_clamped.mean().item():.6f}, std={vec_norm_clamped.std().item():.6f}")
            raise RuntimeError(f"Spherical harmonics calculation failed: {e}")
        if torch.isnan(rsh).any() or torch.isinf(rsh).any():
            print(f"ERROR: RSH output contains NaN or Inf - this should not happen!")
            print(f"vec_normalized stats: mean={vec_normalized.mean().item():.3f}, std={vec_normalized.std().item():.3f}")
            print(f"vec_norm stats: mean={vec_norm.mean().item():.3f}, std={vec_norm.std().item():.3f}")
            print(f"rsh stats: mean={rsh.mean().item():.3f}, std={rsh.std().item():.3f}")
            print(f"rsh shape: {rsh.shape}")
            raise ValueError("NaN/Inf detected in spherical harmonics output - check input normalization")
...
class Message(nn.Module):
    ...
    def forward(self,
        ...
        if torch.isnan(scalar_message).any():
            print(f"Warning: Message - Scalar message contains NaN. Input stats: mean={scalar_mlp_input.mean().item():.3f}, std={scalar_mlp_input.std().item():.3f}")
            scalar_message = torch.nan_to_num(scalar_message, nan=0.0)
        if torch.isnan(spherical_message).any():
            print(f"Warning: Message - Spherical message contains NaN. Input stats: mean={x_spherical_src.mean().item():.3f}, std={x_spherical_src.std().item():.3f}")
...
class Update(nn.Module):
    ...
    def forward(
        ...
        if torch.isnan(new_scalar).any():
            print(f"Warning: Update - Scalar features contain NaN. Input stats: mean={x_scalar.mean().item():.3f}, std={x_scalar.std().item():.3f}")
            new_scalar = torch.nan_to_num(new_scalar, nan=0.0)
        if torch.isnan(new_spherical).any():
            print(f"Warning: Update - Spherical features contain NaN. Input stats: mean={x_spherical.mean().item():.3f}, std={x_spherical.std().item():.3f}")
...
class MultiBodyLayer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super(MultiBodyLayer, self).__init__()
