...
class SPICEDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            target_mean: 能量均值
            target_std: 能量标准差
            grad_target_mean: 力均值
            grad_target_std: 力标准差
            coord_unit: 坐标单位 ("bohr" 或 "angstrom")
            cutoff_unit: 截断距离单位 ("bohr" 或 "angstrom")
            transform: 数据变换
            pre_transform: 预变换
        """
        super().__init__()
    ...
    def _extract_molecule_data(self, molecule: h5py.Group, mol_id: str) -> List[Data]:
        ...
        if 'dft_total_gradient' in molecule:
            forces = molecule['dft_total_gradient'][:]  # [n_confs, n_atoms, 3]
        else:
        ...
        if 'conformations' in molecule:
            conformations = molecule['conformations'][:]  # [n_confs, n_atoms, 3]
        else:
